  ...
  const ImgURL="http://localhost:1337"
   useEffect(()=>{
      ...
      const fetchProduct=async()=>{
         const res = await axios.get(`${baseURL}pull-handles?populate=*`,{
            headers:headers,
        });
      ...
      const fetchFilterProducts=async()=>{
         const res = await axios.get(`${baseURL}pull-handles?populate[pull_handles_product_categories][populate]=*`,{
            headers:headers,
        });
        //  console.log(res.data?.data[0]?.attributes?.pull_handles_product_categories?.data);
        const fetchedCategories = res.data?.data[0]?.attributes?.pull_handles_product_categories?.data
        //  setCategory(res.data?.data[0]?.attributes?.pull_handles_product_categories?.data);
    ...
    <>
        ...
        <div className='main-product1'>
            <div className="product1-banner">
                 <PBannerCard prod={prod?.Information} src={prod?.Image?.data?.attributes?.url}
               //   title="Door Pull Handles" img={pimg} des1={des1} des2={des2} 
                 />
            </div>

            {/* sections */}
            <div className="product1-section1-title" style={{
               textAlign:"left",
               width:"100%",
               padding:"0 8rem"
            }}>By Application</div>
          ...
          >
            Info here
               ...
               <div className="product1-section1-img-wrapper">
                   {
                     Array.isArray(prod?.By_Application?.data) && prod?.By_Application?.data?.map((val,i)=>{
                        return(
          ...
          >
            Info here
               ...
               <div className="product1-section1-img-wrapper">
                   {
                     Array.isArray(prod?.By_Type?.data) && prod?.By_Type?.data?.map((val,i)=>{
                        return(
                           <div className="product1-section1-img-wrapper-w">
                           <img
                              src={`${ImgURL}${val?.attributes?.url}`}
              ...
              >
                ...
                <div className="product1-section3-right">
                     {/* {img3.map((val)=>{
                      ...
                      {
                        Array.isArray(filteredCategories) && filteredCategories?.map((val,i)=>{
                           return(
                              <img
